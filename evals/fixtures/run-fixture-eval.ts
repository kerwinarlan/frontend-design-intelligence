...
import { startFixtureServer } from "./mediocre-app/server.js";
import { inspectWebPage } from "../../scripts/browser-inspector.js";

async function runFixtureEvaluation() {
  console.log("=== Launching Fixture Evaluation Suite ===");
  const server = startFixtureServer(8765);
  console.log("Server running on http://localhost:8765/");

  const beforeDir = path.resolve("evals/fixtures/captures/before");
  const afterDir = path.resolve("evals/fixtures/captures/after");

  try {
    console.log("Capturing Before Fixture screenshots...");
    const beforeResult = await inspectWebPage({
      url: "http://localhost:8765/index.html",
      outputDir: beforeDir,
    });

    console.log("Capturing After (Polished) Fixture screenshots...");
    const afterResult = await inspectWebPage({
      url: "http://localhost:8765/polished.html",
      outputDir: afterDir,
    });

    // Write FIXTURE_REPORT.md
    const reportPath = path.resolve("evals/fixtures/FIXTURE_REPORT.md");
    const reportContent = `# Fixture Evaluation Report: Before vs. After Portfolio Polish
...
* **Before (\`index.html\`)**:
  * Screenshots captured: ${beforeResult.capturedScreenshots.length}
  * Console Errors: ${beforeResult.consoleErrors.length}
  * Layout Overflows: ${beforeResult.layoutOverflows.length}

* **After (\`polished.html\`)**:
  * Screenshots captured: ${afterResult.capturedScreenshots.length}
  * Console Errors: ${afterResult.consoleErrors.length}
  * Layout Overflows: ${afterResult.layoutOverflows.length}
...
`;
    ...
    console.log(`✔ Fixture report generated at ${reportPath}`);
  } finally {
    server.close();
...
runFixtureEvaluation().catch((err) => {
  console.error("Fixture evaluation failed:", err);
  process.exit(1);
});
